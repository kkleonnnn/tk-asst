...
from core import compliance, listing_ms, pricing, scoring, xlsx
...
# ==================== 发布配方（把"该填什么"算死，引擎照抄，省 token） ====================

def publish_recipe(store, pid, variant_names=None):
    """单品「货叮咚发布配方」：一次算出草稿每个字段该填什么，引擎照抄不用边看边想。
    - 文案(标题/描述/卖点)取品卡 listing（M3 生成）；缺则进 warnings。
    - variant_names：采集后从货叮咚读到的中文 SKU 名列表；给了就用字典规则翻成马来（0 token）。
    - 属性：收纳盒类目的货叮咚选项值。价格/重量取选定货源与定价结果。
    绝不含账号信息；SKU 拿不准的词进 warnings 待人工复核。"""
    data = store.load_products()
    card = store.get_product(data, pid)
    if card is None:
        return {"error": f"品卡不存在：{pid}"}
    ...
    pr = card.get("pricing") or {}
    lst = card.get("listing") or {}
    src = next((s for s in (card.get("sources") or [])
                if s.get("id") == card.get("chosen_source_id")), None) or {}
    gd = _is_guangdong(src.get("ship_from"))

    warnings = []
    if not gd:
        warnings.append(f"货源发货地『{src.get('ship_from') or '未知'}』非广东——违反广东货源规则，建议重找广东源。")
    if not (lst.get("title_ms") or lst.get("title_en")):
        warnings.append("缺马来语标题/描述——先在看板填上架素材或派发上架任务。")
    if src.get("weight_g") in (None, ""):
        warnings.append("货源无重量——发货前按尺寸核实（影响头程运费/利润）。")

    skus = None
    if variant_names:
        skus = listing_ms.translate_sku_names_ms(variant_names)
        bad = sorted({u for s in skus for u in s["unknown"]})
        if bad:
            warnings.append("SKU 有拿不准的词待复核：" + "、".join(bad))
    ...
    return {
        ...
        "title_ms": lst.get("title_ms", ""),
        "title_en": lst.get("title_en", ""),
        "desc_ms": lst.get("desc_ms", ""),
        "selling_points_ms": lst.get("selling_points_ms") or [],
        "weight_g": src.get("weight_g"),
        "pricing": {"net_price_myr": pr.get("net_price_myr"),
                    "list_price_myr": pr.get("list_price_myr"),
                    "profit_myr": pr.get("profit_myr")},
        "attributes": listing_ms.storage_box_attributes(),
        "sku_names_ms": skus,
        "warnings": warnings,
    }
