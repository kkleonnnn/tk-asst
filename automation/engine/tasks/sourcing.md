...
## 筛选标准（不合格的不要写入）
- 🔴 **发货地必须在广东省内**（kk 2026-07-08 定规）：达意头程仓在东莞洪梅，广东省内货源头程最短、成本最低、时效最稳。**非广东（如江苏/安徽/浙江/义乌等）的货源不要写入**；筛选时用 1688 的"所在地区/发货地"过滤到广东，或搜索后只留广东卖家。实在只有省外可选时，别硬塞——在 `result_note` 说明"该品广东省内暂无合适源，待人工定夺"，让人决定。
- **支持一件代发**（无货源模式必需）；48h 内发货为佳。
- 进货价合理：结合品卡售价区间倒推——**粗验：进货价(元) × 1.55 ≈ RM 成本，不得逼近售价**；>50 元或大件(>500g)要在 notes 标注运费风险。
- 店铺信誉：回头率/评分/年限可见则记录。
- 重量：尽量从 1688 货源页拿单件重量（商品页「包装信息 → 商品件重尺」表通常列各规格长宽高+重量g）；卖家填的重量可能是占位假数据（如各规格同重），看着不对就别信、按红线标"估计值待核实"。
